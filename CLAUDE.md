...
Env vars (`.env` in repo root, see `.env.example`): `JWT_SECRET` (≥32 chars), `RFP_LLM_PROVIDER` (`anthropic`|`openai`|`deepseek`), and a key per provider — `RFP_LLM_ANTHROPIC_API_KEY`, `RFP_LLM_OPENAI_API_KEY`, `RFP_LLM_DEEPSEEK_API_KEY` — so all three can stay set and switching provider is one line. `RFP_LLM_API_KEY` serves whichever provider has no key of its own. `RFP_LLM_MODEL` and `RFP_LLM_BASE_URL` are optional; blank means the provider's default (`claude-sonnet-4-6` / `gpt-5-mini` / `deepseek-flash`).
...
`AttributeSchemaService` keeps each class's `attribute_def`s in line with the specs its products carry: after every catalog import (touched classes) and via `POST /admin/product-classes/sync-attributes` (all classes). It adds defs for specs on ≥5% of a class's products, types them from the stored values, takes units from the key suffix (`_in`, `_v`, `_pct`…), allows `gte`/`lte` only on numeric specs, and removes identifier defs (`item_no`, `*_item_no`, `mpn`). It also merges duplicate spec names per class (`phases` → `phase`, with value translations like `Single` → `1`): the LLM proposes groups, code vetoes a group whose keys disagree on the same products, products are rewritten to the canonical key, and aliases are kept in `attribute_alias` (+ `attribute_def.value_aliases`) so `canonicalize` translates later imports and tender lines. Spellings of one value within a text spec (`PSC` → `Permanent Split Capacitor`, `90V` → `90`) are merged the same way (`mergeValueSpellings`, every touched class after an import; specs with more than 40 values are skipped as codes), recorded in `value_aliases`. `AccessorySpecs` (run inside `canonicalize` and over stored products on every sync) splits "accessory required" columns such as Grainger's "Capacitor Req." into `<x>_included` (bool), `<x>_required` (bool, false when none is needed) and `<x>_item_no` (the accessory to buy, an identifier). Lettered scales with a real order (`OrderedSpecScales`: insulation class by its temperature rating, IEC efficiency groups) get `gte`, so insulation class H answers a request for F instead of counting as a deviation; the matcher ranks such values when `gte`/`lte` cannot parse a number. Every value merge, whether proposed as a spelling or as part of a key merge, must pass `ValueMergeVeto`: the two must be the same thing written differently (same letters and digits, or the same number with a unit), and a merge is refused whenever one value contains the other, because then one says more — a model proposed `CW` → `CW/CCW`, which made one-way motors reversible. Values naming rotation directions are compared by which directions they claim, so `Clockwise facing output shaft` never becomes `CW/CCW` although they share no words, while `Reversible` may. It is deliberately conservative and also refuses true merges like `Baldor Electric` → `Baldor` and `Class B` → `B` (harmless for `insulation_class`, which `OrderedSpecScales` reads either way). Numbers are read with `SpecNumbers.parse`, which accepts catalog fractions ("13 3/8") but not dual ratings ("115/230").
...
- `OpenAiCompatibleClient` holds the `/chat/completions` shape; `OpenAiLlmClient` and `DeepSeekLlmClient` are just its per-provider differences (base URL, key, defaults). DeepSeek models think by default and the reasoning comes out of the answer's token budget, so that client sends `thinking: {"type":"disabled"}`; any model that answers with no content is retried once with it.
...
- The schema-judgment tasks (`defineClass`, `defineAttributes`, `findDuplicateAttributes`, `findValueSynonyms`) go through `LlmService.judge`, which uses the `rfp.llm.judgment.provider` client when one is configured and the main client otherwise. They judge the catalog rather than read it, and cheap models read well but judge badly, so extraction can run on a cheap model while judgment stays on a stronger one.
