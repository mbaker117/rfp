...
Flyway owns the schema (`ddl-auto: validate`) — every column change needs a new `V{N}__description.sql`. V1/V2 created the original `company`/`instrument` tables; **V3 (`specta_schema`) introduced the current model** and V8–V10 added the crawl tables and product identity key, and V11 the catalog chunk cache. JSON attribute bags (`product.attributes`, `tender_line.attributes`) are `jsonb` columns mapped as `String` with `@JdbcTypeCode(SqlTypes.JSON)`.
